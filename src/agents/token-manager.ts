 ...
 *
 * EDGE RUNTIME COMPATIBLE: Works in both Node.js and Edge Runtime environments.
 *
...
export interface ClaudeCredentials {
  apiKey?: string;
  mcpOAuthTokens?: Record<string, unknown>;
  sessionToken?: string;
}
...
export class TokenManager {
  ...
  private extractApiKeyFromMcpCredentials(credentialsJson: string): string | undefined {
    try {
      ...
      this.log(`Failed to parse credentials JSON: ${String(err)}`);
      return undefined;
    }
  }
  ...
  public getClaudeCredentials(): ClaudeCredentials {
    ...
    const claudeCodeApiKey = this.getKeychainPassword(KEYCHAIN_SERVICES.claudeCode, this.account);
 ...
 */
export function getClaudeCredentials(account?: string, verbose = false): ClaudeCredentials {
...
export function getApiKeyWithPlatformCheck(options: TokenManagerConfig = {}): string | undefined {
  ...
  if (isEdgeRuntime()) {
    if (options.verbose) {
      console.log(
        "[TokenManager] Edge Runtime: using environment variable only - token-manager.ts:339",
      );
    }
